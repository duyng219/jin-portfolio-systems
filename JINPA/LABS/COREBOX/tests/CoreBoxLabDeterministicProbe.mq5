...
#property description "JINPA COREBOX LAB deterministic non-trading probe"
...
void Check(const bool condition, const string testName, const string detail)
{
   ...
   {
      g_passed++;
      Print("[COREBOX_LAB_TEST][PASS] ", testName, " | ", detail);
   }
   else
   {
      g_failed++;
      Print("[COREBOX_LAB_TEST][FAIL] ", testName, " | ", detail);
   }
}
...
void TestDualRendererContract()
{
   ...
   const string highName = "JINPA_COREBOX_LAB_STRUCT_CORE_HIGH";
   const string lowName = "JINPA_COREBOX_LAB_STRUCT_CORE_LOW";
   int ownedLineCount = 0;
   const int lineCount = ObjectsTotal(0, 0, OBJ_HLINE);
   for(int index = 0; index < lineCount; index++)
      if(StringFind(ObjectName(0, index, 0, OBJ_HLINE),
                    "JINPA_COREBOX_LAB_STRUCT_CORE_") == 0)
         ownedLineCount++;
...
void TestCase5RendererPendingHigh()
{
   ...
   const bool ok = ObjectFind(0, "JINPA_COREBOX_LAB_STRUCT_CORE_HIGH") < 0
                   && ObjectFind(0, "JINPA_COREBOX_LAB_STRUCT_CORE_LOW") >= 0
                   && CountOwnedObjects(
                        "JINPA_COREBOX_LAB_STRUCT_CORE_", OBJ_HLINE) == 1
                   && SamePrice(ObjectGetDouble(0,
                        "JINPA_COREBOX_LAB_STRUCT_CORE_LOW", OBJPROP_PRICE), 95.0);
   renderer.Destroy();
...
void TestCase5RendererPendingLow()
{
   ...
   const bool ok = ObjectFind(0, "JINPA_COREBOX_LAB_STRUCT_CORE_HIGH") >= 0
                   ...
                   && CountOwnedObjects(
                        "JINPA_COREBOX_LAB_STRUCT_CORE_", OBJ_HLINE) == 1
                   && SamePrice(ObjectGetDouble(0,
                        "JINPA_COREBOX_LAB_STRUCT_CORE_HIGH", OBJPROP_PRICE), 105.0);
   renderer.Destroy();
...
void TestRepairB6RendererDirectionChange()
{
   ...
   const bool beforeOk = ObjectFind(0,
                            "JINPA_COREBOX_LAB_STRUCT_CORE_LOW") >= 0
                         && ObjectFind(0,
                            "JINPA_COREBOX_LAB_STRUCT_CORE_HIGH") < 0;
   engine.LabProbeClose(94.0, 1030);
   engine.LabProbeClose(93.0, 1031);
   PriceStructureState after = engine.LabProbeState();
   ...
   const bool afterOk = ObjectFind(0,
                        ...
                        && CountOwnedObjects(
                           "JINPA_COREBOX_LAB_STRUCT_CORE_", OBJ_HLINE) == 1
                        && SamePrice(ObjectGetDouble(0,
                           "JINPA_COREBOX_LAB_STRUCT_CORE_HIGH",
                           OBJPROP_PRICE), 108.0);
   renderer.Destroy();
