...
private:
    ...
    string FormatCandleCloseTime(string symbol);
    string StrategyDDKey(string symbol, ulong magicNumber, datetime dayStart);
    double CalculateStrategyDailyDDMoney(string symbol, ulong magicNumber);
...
string CInfoDisplay::FormatCandleCloseTime(string symbol)
{
    int periodSeconds = PeriodSeconds(PERIOD_CURRENT);
    datetime barOpen  = iTime(symbol, PERIOD_CURRENT, 0);
    if(periodSeconds <= 0 || barOpen <= 0)
        return "--:--:--";

    int remaining = (int)(barOpen + periodSeconds - TimeCurrent());
    if(remaining < 0)
        remaining = 0;
...
string CInfoDisplay::StrategyDDKey(string symbol, ulong magicNumber, datetime dayStart)
{
    string date = TimeToString(dayStart, TIME_DATE);
    StringReplace(date, ".", "");
    ...
    return "J23_DD_" + safeSymbol + "_" + IntegerToString((long)magicNumber) + "_" + date;
}

double CInfoDisplay::CalculateStrategyDailyDDMoney(string symbol, ulong magicNumber)
{
    MqlDateTime dt;
    ...
    if(HistorySelect(dayStart, TimeCurrent()))
    {
        for(int i = 0; i < HistoryDealsTotal(); i++)
        {
            ...
            if(magicNumber > 0 && HistoryDealGetInteger(deal, DEAL_MAGIC) != (long)magicNumber) continue;
    ...
    double strategyEquity = realized + floating;
    string key = StrategyDDKey(symbol, magicNumber, dayStart);

    double peak = strategyEquity;
                                     ...
                                     double displayVirtualCapital)
{
    ...
    if(accountEquity <= 0.0)
        accountEquity = AccountInfoDouble(ACCOUNT_BALANCE);

    double strategyDDMoney = CalculateStrategyDailyDDMoney(symbol, magicNumber);
    double strategyDDAcct = (accountEquity > 0.0) ? strategyDDMoney / accountEquity * 100.0 : 0.0;
    double strategyDDVC   = (displayVirtualCapital > 0.0) ? strategyDDMoney / displayVirtualCapital * 100.0 : 0.0;
    double riskVC         = (displayVirtualCapital > 0.0) ?
                            risk * accountEquity / displayVirtualCapital : risk;

    Comment("EA Manual Trading | Magic: ", magicNumber);

    SetText("InfoPoolLine1",
            StringFormat("Bal: %.2f$  |  Eqt: %.2f$",
                         AccountInfoDouble(ACCOUNT_BALANCE),
                         AccountInfoDouble(ACCOUNT_EQUITY)),
            x, y0, 8, textColor);

    SetText("InfoPoolLine2",
            (displayVirtualCapital > 0.0) ?
                StringFormat("Risk: %.2f%% Acct | %.2f%% VC", risk, riskVC) :
                StringFormat("Risk: %.2f%%", risk),
            x, y0 + rowGap, 8, textColor);

    SetText("InfoPoolLine3",
            (displayVirtualCapital > 0.0) ?
                StringFormat("MDD: %.2f%% Acct | %.2f%% VC", strategyDDAcct, strategyDDVC) :
                StringFormat("MDD: %.2f%%", strategyDDAcct),
