...
enum ENUM_RISK_CAPITAL_MODE
{
    RISK_CAPITAL_BALANCE, // Balance
    RISK_CAPITAL_EQUITY,  // Equity
    RISK_CAPITAL_CUSTOM   // Virtual Capital (Custom)
};
...
{
private:
    // Tách nguồn vốn tính risk để sau này mở rộng thêm Fixed Dollar/Portfolio Capital.
    double ResolveRiskCapital(ENUM_RISK_CAPITAL_MODE pRiskMode, double pVirtualCapital);

    // Tính lot theo % risk: lots = (RiskCapital×risk%) / (SL/Point × TickValue)
...
public:
    double MoneyManagement(string pSymbol, ENUM_MONEY_MANAGEMENT pMM, double pMinLotPerEquitySteps,
                           ...
                           ENUM_RISK_CAPITAL_MODE pRiskMode = RISK_CAPITAL_EQUITY,
                           double pVirtualCapital = 10000.0);
...
{
    double volume = 0;
    double riskCapital = ResolveRiskCapital(pRiskMode, pVirtualCapital);
    ...
    switch(pMM)
    {
        ...
        case MM_EQUITY_RISK_PERCENT:
            volume = CalculateVolumeRiskPerc(pSymbol, pRiskPercent, pSLInPricePoints, riskCapital);
            break;
    }
...
double CRiskManager::ResolveRiskCapital(ENUM_RISK_CAPITAL_MODE pRiskMode, double pVirtualCapital)
{
    switch(pRiskMode)
    {
        case RISK_CAPITAL_BALANCE:
            return AccountInfoDouble(ACCOUNT_BALANCE);

        case RISK_CAPITAL_CUSTOM:
            if(pVirtualCapital > 0)
                return pVirtualCapital;

            Print(__FUNCTION__, "() - Invalid VirtualCapital, fallback to Equity");
            return AccountInfoDouble(ACCOUNT_EQUITY);

        case RISK_CAPITAL_EQUITY:
        default:
            return AccountInfoDouble(ACCOUNT_EQUITY);
    }
}
...
double CRiskManager::CalculateVolumeRiskPerc(string pSymbol, double pRiskPercent, double pSLInPricePoints,
                                             double pRiskCapital)
{
    if(pRiskPercent <= 0 || pSLInPricePoints <= 0 || pRiskCapital <= 0)
    {
        Print(__FUNCTION__, "() - Invalid RiskPercent, SL distance, or RiskCapital");
        return 0;
    }
    ...
    double maxRisk     = pRiskPercent * 0.01 * pRiskCapital;
